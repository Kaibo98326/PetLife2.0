      ...
      <div class="info-section">
        ...
        <div class="total-box" style="width: 50%; max-width: 500px; margin-left: auto">
          ...
          >
            ...
            <span class="total-price" style="font-size: 18px; color: #666"
              >$
              {{
                (orderMain.orderTotal + discountAmount + orderMain.usedPoint).toLocaleString()
              }}</span
            >
          </div>
          ...
          >
            ...
            >
              ...
              <span class="total-price" style="font-size: 18px; color: #ff4d4f"
                >- $ {{ discountAmount.toLocaleString() }}</span
              >
            </div>
            <template v-if="isDiscountExpanded">
              <div
                v-for="(ad, index) in appliedDiscounts"
                :key="index"
                style="
                  font-size: 0.9em;
                  margin-bottom: 3px;
              ...
              >
                <span
                  class="label"
                  style="color: #888; background: none; font-weight: normal; padding: 0"
                  >{{ ad.name }}</span
                >
                <span style="color: #ff4d4f"
                  >- $ {{ ad.amount ? ad.amount.toLocaleString() : '0' }}</span
                >
              </div>
            </template>
          </div>
          ...
          >
            ...
            <span class="total-price" style="font-size: 18px; color: #ff4d4f"
              >- $ {{ orderMain.usedPoint.toLocaleString() }}</span
            >
          </div>

          <div
            class="summary-line final-total-line"
            style="
              display: flex;
              justify-content: space-between;
              align-items: center;
          ...
          >
            ...
            <span class="total-price" style="font-size: 20px; font-weight: bold; color: #e67e22"
              >$ {{ orderMain.orderTotal.toLocaleString() }}</span
            >
          </div>
        </div>
