...
public class DiscountEngine {
	...
	public CartCalculateResponseDTO executeDiscount(List<CartItemDTO> originalItems,
			List<Discount> allActiveDiscounts) {
		// 1. 攤平商品
		...
		List<Discount> productLevelDiscounts = allActiveDiscounts.stream()
				.filter(d -> d.getScopeType() != null && d.getScopeType() == 2).collect(Collectors.toList());
		List<Discount> categoryLevelDiscounts = allActiveDiscounts.stream().filter(d -> d.getScopeType() == 1)
	...
	private List<CartItemDTO> mergeCartItems(List<CartItemDTO> flattened) {
		...
		for (CartItemDTO f : flattened) {
			if (!map.containsKey(f.getItemId())) {
				...
				if (f.getProductRole() != null)
					existing.setProductRole(f.getProductRole());
				if (f.getIsActivityMet() != null)
					existing.setIsActivityMet(f.getIsActivityMet());
			}
		}
	...
	private void calculateReminders(List<CartItemDTO> items, List<Discount> allActive) {
		...
		for (CartItemDTO item : items) {
			...
			for (Discount discount : allActive) {
				...
				if (filterEligibleItems(singleItemList, discount, "Main").isEmpty())
					continue;
				...
				if (typeId == 1 || typeId == 2) {
					...
					if (minAmount != null && minAmount.compareTo(BigDecimal.ZERO) > 0
							&& totalAmt.compareTo(minAmount) < 0) {
	...
	private List<CartItemDTO> deepCopyCartItems(List<CartItemDTO> source) {
		...
		for (CartItemDTO item : source) {
			...
			newItem.setProcessed(item.isProcessed()); // 複製當前處理狀態
			copy.add(newItem);
		}
		return copy;
	}
