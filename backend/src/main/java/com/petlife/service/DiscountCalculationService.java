...
public class DiscountCalculationService {
	...
	// 【買 N 送 M】
	public BigDecimal calculateBuyNGetMDiscount(List<CartItemDTO> mainItems, List<CartItemDTO> freeItems,
			Discount discount, boolean isDryRun) {
		...
		if (sets == 0)
			return BigDecimal.ZERO; // 未達門檻直接回傳 0
	...
	// 【條件加購價】
	public BigDecimal calculateAddOnDiscount(List<CartItemDTO> mainItems, List<CartItemDTO> addonItems,
			Discount discount, boolean isDryRun) {
		if (mainItems.size() < discount.getBuyQuantity() || addonItems.isEmpty())
			return BigDecimal.ZERO;
